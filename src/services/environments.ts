...
export async function listEnvironments(): Promise<Record<string, unknown>> {
  ...
  if (runtime) {
    ...
    if (listed.exitCode !== 0) result.container_error = listed.stderr.trim();
  } else {
...
export async function installProot(alias: string): Promise<Record<string, unknown>> {
  requireActions();
  validateName(alias, "发行版别名");
  if (!(await commandExists("proot-distro"))) throw new Error("未安装 proot-distro，请先调用 phone_install_packages 安装 proot-distro。");
  const result = await runCommand("proot-distro", ["install", alias], { timeoutMs: 600_000 });
  if (result.exitCode !== 0) throw new Error(result.stderr || `proot-distro install 退出码 ${result.exitCode}`);
  return { alias, installed: true, output: result.stdout.trim() };
}

export async function removeProot(alias: string, confirm: string): Promise<Record<string, unknown>> {
  requireActions();
  validateName(alias, "发行版别名");
  if (confirm !== `REMOVE:${alias}`) throw new Error(`确认字符串不匹配，应为 REMOVE:${alias}`);
  const result = await runCommand("proot-distro", ["remove", alias], { timeoutMs: 120_000 });
  if (result.exitCode !== 0) throw new Error(result.stderr || `proot-distro remove 退出码 ${result.exitCode}`);
  return { alias, removed: true, output: result.stdout.trim() };
}

export async function execInProot(alias: string, command: string, timeoutSeconds: number): Promise<Record<string, unknown>> {
  requireCommands();
  validateName(alias, "发行版别名");
...
export async function runTermuxCommand(command: string, timeoutSeconds: number): Promise<Record<string, unknown>> {
  requireCommands();
  const result = await runCommand("bash", ["-lc", command], { timeoutMs: timeoutSeconds * 1_000 });
  return { ...result };
}

export async function installPackages(packages: string[]): Promise<Record<string, unknown>> {
  requireActions();
  ...
  if (result.exitCode !== 0) throw new Error(result.stderr || `pkg install 退出码 ${result.exitCode}`);
  return { installed: unique, output: result.stdout.trim() };
}

export async function manageCompose(
  runtime: "auto" | "docker" | "podman",
  projectDirectory: string,
  action: "pull" | "up" | "restart" | "stop" | "down",
  confirm?: string,
  ...
  const selected = runtime === "auto"
    ...
    : runtime;
  if (!selected || !(await commandExists(selected))) throw new Error("未找到可用的 Docker/Podman 运行时。");
  ...
  if (result.exitCode !== 0) throw new Error(result.stderr || `${selected} compose ${action} 失败`);
  return { runtime: selected, project_directory: cwd, action, output: result.stdout.trim() || result.stderr.trim() };
}

export async function prepareServiceRoot(): Promise<string> {
  requireActions();
...
function validateName(value: string, label: string): void {
  if (!SAFE_NAME.test(value)) throw new Error(`${label}只能包含小写字母、数字、点、下划线和短横线，且最长 64 个字符。`);
}
