...
export async function deletePath(target: string, recursive: boolean, confirm: string): Promise<Record<string, unknown>> {
  requireActions();
  const resolved = resolveWithinHome(target);
  if (resolved === resolveWithinHome("~")) throw new Error("拒绝删除 Termux HOME 本身。");
  const base = path.basename(resolved);
  if (confirm !== `DELETE:${base}`) throw new Error(`确认字符串不匹配，应为 DELETE:${base}`);
  const info = await stat(resolved);
  if (info.isDirectory() && !recursive) throw new Error("目标是目录，删除目录需要 recursive=true。");
  await rm(resolved, { recursive, force: false });
  return { path: resolved, deleted: true, type: info.isDirectory() ? "dir" : "file" };
}

export async function movePath(from: string, to: string): Promise<Record<string, unknown>> {
  requireActions();
...
async function collectFiles(root: string, depth: number, out: string[], maxFiles: number): Promise<void> {
  ...
  for (const dirent of dirents) {
    ...
    if (dirent.name === "node_modules" || dirent.name === ".git" || dirent.name === ".cache") continue;
    const full = path.join(root, dirent.name);
    if (dirent.isDirectory()) await collectFiles(full, depth + 1, out, maxFiles);
