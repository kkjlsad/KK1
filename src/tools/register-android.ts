...
export function registerAndroidTools(server: McpServer): void {
  server.registerTool(
    "phone_get_notifications",
    {
      title: "读取通知栏",
      description:
        "读取当前通知栏里的通知（含 App 包名、标题、内容）。需要在系统设置里给 Termux:API 开“通知使用权”。只读，内容可能含隐私信息。",
  ...
  server.registerTool(
    "phone_send_sms",
    {
      title: "发送短信",
      description:
        "用手机发送一条短信。需要 PHONE_MCP_ALLOW_ACTIONS=1 与短信发送权限。此操作不可撤回，建议在 RikkaHub 为此工具开启每次确认。",
      inputSchema: z.object({
  ...
  server.registerTool(
    "phone_camera_photo",
    {
      title: "用手机相机拍照",
      description:
        "调用手机相机拍一张照片并保存到 Termux HOME 内。需要 PHONE_MCP_ALLOW_ACTIONS=1 及相机权限。存到 ~/storage/shared/DCIM 会直接进相册。",
      inputSchema: z.object({
  ...
  server.registerTool(
    "phone_list_installed_apps",
    {
      title: "列举已装应用（尽力而为）",
      description:
        "未 root 时 Android 不允许读取完整应用列表。本工具通过共享存储 /sdcard/Android/media 与 obb 下的包名目录推断，结果不完整。只读。",
