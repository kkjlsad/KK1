...
export function registerWorkspaceTools(server: McpServer): void {
  server.registerTool(
    "phone_edit_file",
    {
      title: "编辑文本文件（精确替换）",
      description:
        "把 Termux HOME 内文本文件中的 find 替换为 replace。默认要求 find 在文件中唯一（避免误改），需要批量时显式设置 all=true。需要 PHONE_MCP_ALLOW_ACTIONS=1。",
  ...
  server.registerTool(
    "phone_search_files",
    {
      title: "搜索文件或文件内容",
      description:
        "在 Termux HOME 内按文件名（name）或内容（content，支持正则）搜索。自动跳过 node_modules、.git、.cache 与大于 1MB 的文件。只读。",
      inputSchema: z.object({
    ...
    async ({ path, name, content, max_results }) => wrap(async () => {
      if (!name && !content) throw new Error("name 和 content 至少要提供一个。");
  ...
  server.registerTool(
    "phone_delete_path",
    {
      title: "删除文件或目录",
      description:
        "删除 Termux HOME 内的文件或目录，必须提供 DELETE:<名字> 形式的确认字符串；删除目录还要 recursive=true。此操作不可恢复，建议在 RikkaHub 开启每次确认。",
      inputSchema: z.object({
  ...
  server.registerTool(
    "phone_git",
    {
      title: "执行 Git 操作",
      description:
        "在 Termux HOME 内的 Git 仓库执行操作。只读：status/diff/log/branch/remote/show/rev-parse；写入（需 PHONE_MCP_ALLOW_ACTIONS=1）：add/commit/push/pull/fetch/stash。额外参数用 args 数组传入，不经 shell 解析。",
      inputSchema: z.object({
  ...
  server.registerTool(
    "phone_clone_repo",
    {
      title: "克隆远程仓库",
      description:
        "把远程 Git 仓库克隆到 Termux HOME 内。支持 https:// 与 git@ 地址（SSH 需本机已配置密钥）。需要 PHONE_MCP_ALLOW_ACTIONS=1。",
      inputSchema: z.object({
