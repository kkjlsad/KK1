...
export function registerTools(server: McpServer): void {
  ...
  server.registerTool(
    ...
    async ({ duration_seconds, interval_seconds }) => {
      if (interval_seconds > duration_seconds) return failure("采样间隔不能大于总时长。", "减小 interval_seconds。");
  ...
  server.registerTool(
    ...
    async ({ runtime, project_directory, action, confirm }) => wrap(
      () => manageCompose(runtime, project_directory, action, confirm),
    ),
  );
  ...
  server.registerTool(
    ...
    async ({ brightness }) => wrap(async () => {
      if (!ALLOW_ACTIONS) throw new Error("写操作未启用，请设置 PHONE_MCP_ALLOW_ACTIONS=1 后重启服务。");
      if (!(await commandExists("termux-brightness"))) throw new Error("Termux:API 命令不可用。");
      const result = await runCommand("termux-brightness", [String(brightness)], { timeoutMs: 12_000 });
      if (result.exitCode !== 0) throw new Error(result.stderr || "亮度调整失败");
  ...
  server.registerTool(
    ...
    async ({ stream, volume }) => wrap(async () => {
      if (!ALLOW_ACTIONS) throw new Error("写操作未启用，请设置 PHONE_MCP_ALLOW_ACTIONS=1 后重启服务。");
      if (!(await commandExists("termux-volume"))) throw new Error("Termux:API 命令不可用。");
      const result = await runCommand("termux-volume", [stream, String(volume)], { timeoutMs: 12_000 });
      if (result.exitCode !== 0) throw new Error(result.stderr || "音量调整失败");
