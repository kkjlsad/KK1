...
export function registerFileTools(server) {
    server.registerTool("phone_list_files", {
        title: "列出目录内容",
        description: "列出 Termux HOME 内的目录，返回条目名称、类型、大小和修改时间。~/storage 下是手机共享存储（相册、下载等）。只读。",
    ...
    server.registerTool("phone_read_file", {
        title: "读取文本文件",
    ...
    server.registerTool("phone_write_file", {
        title: "写入文本文件",
        description: "把文本写入 Termux HOME 内的文件，父目录会自动创建。append=false 时会覆盖原文件。需要 PHONE_MCP_ALLOW_ACTIONS=1。",
        inputSchema: z.object({
            path: z.string().min(1).max(500).describe("文件路径，必须位于 Termux HOME 内"),
    ...
    server.registerTool("phone_download", {
        title: "下载文件到手机",
        description: "用 curl 把 http/https 链接下载到 Termux HOME 内的路径。需要 PHONE_MCP_ALLOW_ACTIONS=1。存到 ~/storage/shared/Download 就会出现在手机下载目录。",
        inputSchema: z.object({
...
//# sourceMappingURL=register-files.js.map