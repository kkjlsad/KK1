...
export async function listFiles(target) {
...
export async function readTextFile(target, maxBytes) {
    const resolved = resolveWithinHome(target);
    const info = await stat(resolved);
    if (info.isDirectory())
        throw new Error(`${resolved} 是目录，请改用 phone_list_files。`);
    const limit = Math.min(Math.max(maxBytes, 256), 200_000);
    const raw = await readFile(resolved);
    const clipped = raw.subarray(0, limit);
    if (clipped.includes(0))
        throw new Error("这是二进制文件，本工具只读文本。");
    return {
        path: resolved,
...
export async function writeTextFile(target, content, append) {
    requireActions();
    const resolved = resolveWithinHome(target);
...
export async function downloadFile(url, destination, timeoutSeconds) {
    requireActions();
    let parsed;
    try {
        parsed = new URL(url);
    ...
    await mkdir(path.dirname(resolved), { recursive: true });
    const result = await runCommand("curl", ["-fL", "--retry", "2", "--max-time", String(timeoutSeconds), "-o", resolved, url], { timeoutMs: (timeoutSeconds + 10) * 1_000 });
    if (result.exitCode !== 0)
        throw new Error(result.stderr.trim() || `curl 退出码 ${result.exitCode}`);
    const info = await stat(resolved);
