...
export async function getPhoneOverview() {
...
export async function samplePower(durationSeconds, intervalSeconds) {
    const samples = [];
    const startedAt = Date.now();
    const deadline = startedAt + durationSeconds * 1_000;
    do {
        samples.push(await getBatteryStatus());
        if (Date.now() + intervalSeconds * 1_000 > deadline)
            break;
        await delay(intervalSeconds * 1_000);
    } while (Date.now() < deadline);
    const powers = samples.map((item) => item.estimated_power_w).filter((value) => value !== undefined);
    const currents = samples.map((item) => item.current_ua).filter((value) => value !== undefined);
    return {
...
export async function getResourceStatus(sampleMilliseconds) {
...
async function getAndroidProperties(keys) {
    if (!(await commandExists("getprop")))
        return Object.fromEntries(keys.map((key) => [key, null]));
    const pairs = await Promise.all(keys.map(async (key) => {
        const result = await runCommand("getprop", [key], { timeoutMs: 3_000 });
        return [key, result.exitCode === 0 ? result.stdout.trim() || null : null];
    }));
    return Object.fromEntries(pairs);
}
function normalizeTermuxBattery(raw) {
...
async function getSysfsBattery() {
...
async function readMemInfo() {
    ...
    if (!text)
        return { total_bytes: os.totalmem(), available_bytes: os.freemem(), used_percent: null };
...
async function readCpuFrequencies() {
