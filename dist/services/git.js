...
const WRITE_ACTIONS = ["add", "commit", "push", "pull", "fetch", "stash"];
export async function gitAction(repository, action, args, message) {
    ...
    switch (action) {
        ...
        case "commit":
            if (!message)
                throw new Error("commit 需要提供 message。");
...
export async function cloneRepo(url, destination, depth) {
    requireActions();
